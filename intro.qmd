...
```{r full pixel, echo=FALSE,fig.height=2, fig.width=4, fig.cap= "100% pixel display when getting exploring SNAP"}
# cat("<style>p.caption</style>")
...
```{r tasseled scatter, echo=FALSE,fig.height=2, fig.width=4, fig.cap= "Bucharest Spectral Feature - tasseled cap scatterplot", results='asis'}
# cat("<style>p.caption</style>")
...
In my first results, I made the mistake of comparing the wrong bands, comparing B8 in S-2 and B6 in Landsat which do not measure similar aspects. For this reason, the Band 4 in the above table fo not make sense, in the Sentinel Observations the grass values being much too high! 
Furthermore, from a visualisation  perspective, the colors are not coordinated with the usually considered colours (i.e. grass as green) which make the presentation of the results more confusing.

All in all, this first appraoch was beneficial and very much enjoyed! Understanding how both S-2 and L-8 bands function was not always easy, but with trial and error the [final results](### Computational Analysis of L-8 and S-2 Images) make sense! We were able to show comparisons between L-8 and S-2 for locally defined POI's and provided a good initiation in understanding similarities and differences between platforms. 
